...
const CreateBody = z.object({
  ...
  requestFields: z.string().max(50000).nullable().optional(),
  mockFields: z.string().max(50000).nullable().optional(),
  responseMode: z.enum(["inherit", "custom", "raw"]).default("inherit").optional(),
  customWrapper: z.string().max(10000).nullable().optional(),
...
export async function POST(req: NextRequest) {
  ...
  const row = {
    ...
    requestFields: parsed.data.requestFields ?? null,
    mockFields: parsed.data.mockFields ?? null,
    responseMode: parsed.data.responseMode ?? "inherit",
    customWrapper: parsed.data.customWrapper ?? null,
